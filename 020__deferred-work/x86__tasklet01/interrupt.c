...
/* tasklet in static memory, or dynamic allocated? */
#define INTERRUPT_STATIC_TASKLET     777
...
#else
struct tasklet_struct *tasklet; /* NB: don't initialize globals to NULL */
#endif
...
int init_hello_interrupt(void)
{
	...
	if (!tasklet)
		goto err_irq; /* don't throw messages here */
...
void cleanup_hello_interrupt(void)
{
	...
	/* NB: no check like 'if (tasklet)' since kfree(NULL) is safe */
	kfree(tasklet);
