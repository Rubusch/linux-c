...
/*
  ...
  This is the SPI version of Alberto Liberal's accelerometer driver
...
{
	__le16 buf[3];
	dev_info(ac->dev, "%s() - called", __func__);
...
adxl345_irq(int irq, void *handle)
{
	...
	*/
	...
	if (int_stat & (SINGLE_TAP)){
		dev_info(ac->dev, "%s() - single tap interrupt has occurred", __func__);
...
adxl345_probe(struct device *dev, const struct adxl345_bus_ops *bops)
{
	...
	if (!ac) {
		dev_err(dev, "%s() - FAILED to allocate memory", __func__);
		err = -ENOMEM;
		goto err_out;
	}
...
// FIXME, revid is 0x00, thus fails here!!!    
	revid = AC_READ(ac, ADXL345_REG_DEVID);            
	dev_info(dev, "%s() - DEVID: 0x%02X", __func__, revid);
	dev_info(dev, "%s() - dev_name(dev) '%s'", __func__, dev_name(dev));  
	
	if (revid == 0xE5) {
		dev_info(dev, "%s() - ADXL345 is found", __func__);
	} else {
		dev_err(dev, "%s() - failed to probe %s", __func__, input_dev->name);
		err = -ENODEV;
		goto err_out;
	}

	snprintf(ac->phys, sizeof(ac->phys), "%s/input0", dev_name(dev));
	dev_info(dev, "%s() - ac->phys '%s'", __func__, ac->phys);  
	...
	if (IS_ERR(ac->gpio)) {
		dev_err(dev, "%s() - gpio get index failed", __func__);
	...
	if (ac->irq < 0) {
		dev_err(dev, "%s() - gpio get irq failed", __func__);
		err = ac->irq;
		goto err_out;
	}
	dev_info(dev, "%s() - the IRQ number is: %d", __func__, ac->irq);
...
static int adxl345_spi_probe(struct spi_device *spi)
{
	...
	dev_info(&spi->dev, "%s() - called", __func__);
