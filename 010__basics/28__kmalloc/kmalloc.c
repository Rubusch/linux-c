...
static int __init mod_init(void) {
	...
	if (NULL == ptr1) {
		pr_err("%s(): failed to kmalloc() ptr1\n", __func__);
		return -ENOMEM;
	}
	pr_info("%s(): *ptr1: 0x%08x - kmalloc()\n", __func__, *ptr1);

	*ptr1 = 0xc00abcde;
	pr_info("%s(): *ptr1: 0x%08x\n", __func__, *ptr1);
	kfree(ptr1);

	// ptr1 - kzalloc()
	ptr1 = kzalloc(sizeof(*ptr1), GFP_KERNEL);
	if (NULL == ptr1) {
		pr_err("%s(): failed to kzalloc() the ptr1\n", __func__);
		return -ENOMEM;
	}
	pr_info("%s(): *ptr1: 0x%08x - kzalloc()\n", __func__, *ptr1);
	...
	*ptr1 = 0xc0012345;
	...
	if (NULL == ptr2) {
		pr_err("%s(): failed to kzalloc() ptr2\n", __func__);
		return -ENOMEM;
	}
	ptr2->val = 123;
